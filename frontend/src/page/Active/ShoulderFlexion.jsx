...
export default function ShoulderFlexion({
    ...
    const baseTitle = "Shoulder Flexion (ยกแขนไปหน้า)";
    const sideText = selectedArm === 'left' ? 'ด้านซ้าย' : 'ด้านขวา';
    const modeTitle = baseTitle.replace(')', sideText + ')');
    const trackingMode = "shoulder";
    const description = "ยกแขนขึ้นไปด้านหน้าให้สูงที่สุด";
            ...
            .then(res => {
                if (res.data) {
                    ...
                    if (mode) {
                        ...
                        // fallback ถ้าหา slug แยกข้างไม่เจอ ให้ใช้ ID จาก ROM Test (15, 16)
                        setTherapyId(selectedArm === 'right' ? 15 : 16);
                    }
                }
            })
            .catch(err => {
    ...
    const handleFinish = useCallback(async () => {
        ...
        setIsFinished(true);
    }, [currentCount, timeElapsed, targetCount, patientId, isRoutineMode, onComplete]);
    ...
    return (
        <div className="w-full h-[calc(100vh-73px)] flex flex-col overflow-hidden">
            ...
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 invisible">
                {/* Placeholder for modal structure alignment if needed */}
            </div>
