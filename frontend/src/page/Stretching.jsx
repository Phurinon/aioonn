    ...
    const handleToggleTracking = () => {
        if (mediapipeRef.current) {
            if (isTracking) {
                ...
                const flexScore = Math.floor(Math.random() * 35) + 65; // 65-100 คะแนน
                const newSession = {
                    id: Date.now(),
                    timestamp: new Date(),
                    duration: duration,
                    score: flexScore,
                    flexibility: flexScore >= 85 ? "ยืดหยุ่นดี" : flexScore >= 70 ? "ปานกลาง" : "ควรยืดเหยียดบ่อยขึ้น",
                    note: "การยืดเหยียด" + (flexScore >= 75 ? "ดี" : "ต้องทำบ่อยขึ้น")
                };
